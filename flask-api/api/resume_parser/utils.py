...
def extract_website(text):
    '''
    Helper function to extract website from text

    :param text: plain text extracted from resume file
    '''
    website = re.findall('((www\.[^\s]+)|(https?://[^\s]+))', text)

    #linkedin\.com\/(pub|in|profile)
...
def get_job_desc_skills(text):
    nlp = spacy.load('en_core_web_sm')
    nlp_text = nlp(text)
    return extract_skills(nlp_text, nlp_text.noun_chunks)
    


def extract_sections(resume_text):
    section_pattern = r"\b[A-Z]{2,}\b\n"
    # Find all matches for section headings in the resume text using the regular expression pattern
    section_matches = re.findall(section_pattern, resume_text)
    return [s.strip() for s in section_matches]
